                ...
                <div class="border rounded-xl shadow-lg p-4 bg-white hover:shadow-2xl transition">
                    ...
                    <p class="text-sm mt-2">
                        ...
                        <span class="px-2 py-1 rounded text-white 
                            {{ $booking->status === 'approved' ? 'bg-green-500' : ($booking->status === 'pending' ? 'bg-yellow-500' : 'bg-green-500') }}">
                    ...
                    @if($booking->status !== 'cancelled')
                        <form action="{{ route('public.bookings.cancel', $booking->id) }}" method="POST" class="mt-3">
                            @csrf
                            @method('DELETE')
                            <button type="submit" 
